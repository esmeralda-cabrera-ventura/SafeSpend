...
* Using SafeSpend to Generate Financial Trend Graphs

If you want to use SafeSpend to create multi-month financial trend graphs, all financial data must be entered and saved within a single active session. The app temporarily stores each month’s inputs in a shared CSV file, which allows SafeSpend to generate charts and identify spending or savings patterns over time.

* How to correctly generate trend graphs:

Open the app and start a new session.

Enter financial data one month at a time (e.g., January → Save, February → Save, March → Save).

After entering all desired months, view the graphs and insights generated by the app.

When you are finished, you must press “Reset Data” before closing the app.

* Why this is important:

SafeSpend uses a single shared CSV file in the app environment.
Because there are no user accounts or private storage, financial entries are not isolated between users. Resetting the data ensures that:

Your financial information is cleared

No one else accessing the app link can see your entries

The shared file does not retain personal data after your session ends
